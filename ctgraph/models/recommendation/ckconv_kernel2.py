...
class CKConv(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if x_shape[-1] < self.train_length.item():
            ...
            return ckconv.causal_conv(x, conv_kernel, self.bias)
        else:
            # Otherwise use fft convolution:
            return ckconv.causal_fftconv(x, conv_kernel, self.bias)
    ...
    @staticmethod
    def calculate_max(train_length, current_length):
        """
