...
class GATv2CKGConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # The kernel (linear operation) should have the same variance as self.lin_l, which is glorot initialized
        # so we have var=1/12(2*sqrt(6/in_channels/2))**2
        # with std being the sqrt of that
        # see https://en.wikipedia.org/wiki/Continuous_uniform_distribution
        # and https://pytorch.org/docs/stable/nn.init.html
        kernel_std = math.sqrt(1/12 * (2*math.sqrt(6/in_channels/2))**2)
        self.siren = KernelNet(1, in_channels ** 2, kernel_hidden_size, 'Sine', 'LayerNorm', 1, bias, omega_0,
                               dropout, kernel_std)
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
        ...
        t = edge_attr
        kernels = self.siren(t.view(-1, 1, 1)).view((len(t), self.in_channels, self.in_channels))
        x_j_ = torch.einsum('abc,adc->adb', kernels, x_j)

        # So the root node is transformed with a standard linear transform, since it doesn't have a temporal
        # relation with itself
        x = self.lin_r(x_i) + x_j_
