...
class CKConv(nn.Module): # Continuous Kernel Convolution
    ...
    def forward(self, u_embedded, i_embedded, user_per_trans, item_per_trans, edges_t):

        # make times relative
        newest_time = edges_t.max()
        rel_time = newest_time - edges_t

        # get kernels
        user_kernels = self.w_users(rel_time).reshape((len(edges_t), self.hidden_size, self.hidden_size))
        item_kernels = self.w_items(rel_time).reshape((len(edges_t), self.hidden_size, self.hidden_size))
        ...
        user_messages = (user_kernels @ i_embedded[user_per_trans].unsqueeze(-1)).squeeze()
