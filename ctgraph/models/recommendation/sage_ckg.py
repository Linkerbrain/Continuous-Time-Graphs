...
class SAGECKGConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.ckg:
            ...
            # The kernel (linear operation) should maintain the input variance of 1 after it is applied
            # So this needs to be the std of the kernel
            kernel_std = math.sqrt(1/in_channels[0])
            self.siren = KernelNet(1, in_channels[0]**2, kernel_hidden_size, 'Sine', 'LayerNorm', 1, bias, omega_0,
                                     dropout, kernel_std)
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj, edge_attr: Tensor,
        ...
        # if self.ckg:
            ...
            # kernels = self.siren(edge_time.view(-1, 1, 1)).view((len(edge_time), self.in_channels[0], self.in_channels[0]))
            # x_l = (kernels @ x[0].unsqueeze(-1)).squeeze()
            # x = (self.lin(x[0]).relu(), x[1])
